#!/usr/bin/env python
"""
大学物理 Manim 渲染管线（独立于 render.py / render_chemistry.py，互不干扰）。

为什么独立：render.py 的 write_generated 不写 subjectId，且会整体重写
media.generated.ts；且物理 chapterId（ch02-ch14）与概率论/化学的 chapterId 重叠，
绝不能共用扫描目录。为避免破坏概率论/化学视频，本脚本只渲染 manim/physics/ 下的
场景，产物落 public/media/videos/physics/，并写入**独立**的
content/media.physics.generated.ts（条目带 subjectId="physics"）。
media.ts 会把三份清单合并。

场景约定：每个 manim/physics/<chXX>/*.py 模块顶层导出 REGISTER 列表：
    REGISTER = [{
        "scene": "Ch07Kp1CoulombLaw", "id": "phys-ch07-7.1-kp1-coulomb-law",
        "chapterId": "ch07", "sectionId": "7.1",
        "title": "...", "description": "...",
    }]

用法：
    python manim/render_physics.py                 # 渲染全部缺失视频
    python manim/render_physics.py --chapter ch07  # 仅某章
    python manim/render_physics.py --force         # 强制重渲
"""
...
SCENES_DIR = ROOT / "manim" / "physics"
MEDIA_OUT = ROOT / "manim" / "media_physics"
PUBLIC_VIDEOS = ROOT / "public" / "media" / "videos" / "physics"
GENERATED_TS = ROOT / "lib" / "content-data" / "media.physics.generated.ts"
...
def write_generated(entries: list[dict]) -> None:
    ...
    GENERATED_TS.write_text(
        ...
        f"export const physicsVideos: VideoEntry[] = {body};\n",
        encoding="utf-8",
    )
...
def main() -> int:
    ...
    for py in discover_modules(args.chapter):
        ...
        for e in reg:
            ...
            if dest.exists():
                e["src"] = f"/media/videos/physics/{e['chapterId']}/{e['id']}.mp4"
