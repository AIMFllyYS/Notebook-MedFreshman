...
class Ch12Kp3BohrOrbitQuantization(Scene):
    def construct(self):
        ...
        for circ, lbl in zip(orbits, orbit_n_labels):
        ...
        self.play(Create(wave2), FadeIn(standing_text), run_time=1.0)
        self.wait(0.5)
        self.play(Create(wave3), run_time=0.8)
        self.wait(1.2)
        ...
        # 右侧数值面板
        def n_val():
            return n_tracker.get_value()

        panel_x = 1.5

        n_readout = always_redraw(
            lambda: VGroup(
                Text("n  =", font=CJK, color=WHITE).scale(0.46),
                MathTex(rf"{n_val():.2f}", color=ORANGE).scale(0.72),
            ).arrange(RIGHT, buff=0.2).move_to([panel_x + 1.2, 1.6, 0])
        )
        r_readout = always_redraw(
            lambda: VGroup(
                Text("r  =", font=CJK, color=YELLOW).scale(0.46),
                MathTex(rf"{n_val()**2:.2f}\ a_0", color=YELLOW).scale(0.72),
            ).arrange(RIGHT, buff=0.2).move_to([panel_x + 1.2, 0.7, 0])
        )
        L_readout = always_redraw(
            lambda: VGroup(
                Text("L  =", font=CJK, color=GREEN).scale(0.46),
                MathTex(rf"{n_val():.2f}\ \hbar", color=GREEN).scale(0.72),
            ).arrange(RIGHT, buff=0.2).move_to([panel_x + 1.2, -0.2, 0])
        )
        v_readout = always_redraw(
            lambda: VGroup(
                Text("v  =", font=CJK, color=CYAN).scale(0.46),
                MathTex(rf"v_1/{n_val():.2f}", color=CYAN).scale(0.72),
            ).arrange(RIGHT, buff=0.2).move_to([panel_x + 1.2, -1.1, 0])
        )
        ...
        self.add(n_readout, r_readout, L_readout, v_readout)
        ...
        self.play(n_tracker.animate.set_value(4.0), run_time=4.0, rate_func=linear)
        self.wait(0.8)
        ...
        self.remove(n_readout, r_readout, L_readout, v_readout)
        ...
        for b in q_bars:
            self.play(Create(b), run_time=0.28)
