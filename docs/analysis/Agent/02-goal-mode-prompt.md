...
每轮取一个 Issue，走完三步再取下一个：

1. **修复** —— 读 Issue 正文里的 `file:line`，理解现状，改代码，跑 `pnpm test`
2. **验收** —— 用全新视角对照 Issue 的「验收标准」逐条核对，跑 `pnpm test` 和 `pnpm lint`
3. **返修** —— 验收有不通过项就修，修完自己补完剩余验收，闭环

取 Issue 的规则：从 #48–#101 中，选出 `depends-on` 列出的 Issue 全部已关闭的那些，
按 Issue 号从小到大取。正文末尾的 yaml 块里有 `depends-on` 和 `conflict-domain`。
打了 `blocked` 标签的跳过。
