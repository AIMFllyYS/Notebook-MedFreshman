...
> 本 SOP 的出题阶段必须遵守 [00-infrastructure.md「内容生产闭环与反降质契约」](./00-infrastructure.md#内容生产闭环与反降质契约)：**禁止用脚本/模板批量生成题目**——每道题的立意、干扰项设计、答案核验都需要理解具体知识点，脚本只能做 Step 5 的 JSON 格式化，不能替代出题和核验这两步判断。出题与核验必须闭环配对（见下文「出题→核验闭环」），不能只出题不核验就交付。
...
`lib/stores/ui.ts` 的 `setActiveRoute()` 按板块声明的 `keyStrategy` 推导 Quiz key；录音板块使用 `category-item` 策略，直接把 `itemId`（如 `"rec-01"`）作为 Quiz key，使 QuizTab 能从 `content/quiz/{subject}/rec-01.json` 加载数据。无需修改 loader 或 progress 逻辑。
