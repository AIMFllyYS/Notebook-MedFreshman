...
**本轮 Agent 平台整改（`docs/plans/Agent-refactor/`，2026-09-12 起重排）**

- **调度以 loop 制为准。** `docs/plans/Agent-refactor/00-loop-map.md` 是这一轮「做了什么 / 按什么顺序做」的入口；`docs/analysis/Agent/01-goal-mode-runbook.md` 第 1–3 节的「一个 Issue = 一个 loop」已被取代，不要再切 54 个循环。
- **`proxy.ts` 的 `config.matcher` 必须是编译期字面量数组**（付费路径写死）。不要改回 `[...PAID_AI_API_PATHS]` 或任何 spread / 跨模块引用——那是 `next dev` 全站 500 的直接原因。闸门逻辑仍走 `lib/auth/aiGate.ts` 的 `decideAiGate` + `isPaidAiApiPath`，两边值一致由测试钉住。
...
- **计价一律按实际落地模型。** `selected_model_id` 记用户选的，`actual_model_id` 记真正打的。新增花钱调用必须带正确的 `route` 与 `kind`。
